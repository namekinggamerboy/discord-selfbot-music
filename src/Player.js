...
const { VoiceChannel, version } = require("discord.js-selfbot");
...
class Player {
 ...
 seek(voiceChannel, songName, requestedBy, guildID, gf) {
        this.queues = this.queues.filter((g) => g.guildID !== voiceChannel.id);
        return new Promise(async (resolve, reject) => {
...
async _playSong(guildID, firstPlay) {
        ...
        var dispatcher = queue.connection.play(ytdl(song.url, { filter: "audioonly", quality: "highestaudio", highWaterMark: 1 << 25  }),{ seek: queue.seek||0 });
